...
본 프로젝트와 유사하게 EEG 뇌파 신호를 Spectrogram 이미지로 변환하여 분석하는 접근 프로젝트를 진행하고자 하기에 이것을 참고 프로젝트로 선정함. 특히 EEG 데이터를 이미지 형태로 변환하여 이미지 기반 딥러닝 모델의 성능을 향상시키는 방법을 제시하고 있어 집중 상태와 휴식 상태를 분류하는 본 프로젝트에 적용 가능한 방법론을 제공할 것으로 기대됨.
